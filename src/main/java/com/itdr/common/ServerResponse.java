...
public class ServerResponse<T> implements Serializable {
    ...
    private ServerResponse(String msg) {
        this.status = Const.DEFAULT_FAIL;
        this.msg = msg;
    }


    public static <T> ServerResponse successRS() {
        return new ServerResponse(Const.DEFAULT_SUCCESS);
    }

    public static <T> ServerResponse successRS(String msg) {
        return new ServerResponse(Const.DEFAULT_SUCCESS,msg);
    }

    public static <T> ServerResponse successRS(T data) {
        return new ServerResponse(Const.DEFAULT_SUCCESS,data);
    }
    ...
    public static <T> ServerResponse defeatedRS(String errorMessage) {
        return new ServerResponse(Const.DEFAULT_FAIL,errorMessage);
    }

    @JsonIgnore
//让success不在json序列化结果之中
    public boolean isSuccess() {
        return this.status == Const.DEFAULT_SUCCESS;
    }
}
