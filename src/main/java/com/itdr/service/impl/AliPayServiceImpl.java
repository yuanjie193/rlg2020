...
public class AliPayServiceImpl implements AliPayService {
    ...
    public ServerResponse pay(Users user, Long orderNo) {
        ...
        if(order == null){
            return ServerResponse.defeatedRS(Const.DEFAULT_FAIL,Const.aliPayEnum.ORDER_UN_EXIT.getDesc());
        }
        /*//判断订单是否匹配
        if(order.getUserId() != user.getId()){
            return ServerResponse.defeatedRS(Const.DEFAULT_FAIL,Const.aliPayEnum.ORDER_UN_MATCH.getDesc());
        }*/
        //根据订单号查询对应的商品详情
       List<OrderItem> orderItemList = orderItemMapper.selectByOrderNoAndUserID(orderNo,user.getId());
        ...
        try {
            ...
            return ServerResponse.defeatedRS
                    (Const.aliPayEnum.FAIDED_ORDER.getCode(),Const.aliPayEnum.FAIDED_ORDER.getDesc());
        }
        ...
        if (response != null && response.isSuccess()) {
            ...
            return ServerResponse.defeatedRS(Const.aliPayEnum.FAIDED_ORDER.getCode(),Const.aliPayEnum.FAIDED_ORDER.getDesc());
        }
    ...
    @Override
    public ServerResponse queryOrderPayStatus(Users user, Long orderNo) {
        if(orderNo ==null || orderNo <0){
            return ServerResponse.defeatedRS(Const.DEFAULT_FAIL,Const.UNLAWFULNESS_PARAM);
        }
        //判断订单是否存在
